...
load('brainNetSet_SRTL_NITRC.mat'); %Brain network sets based on different net parameters.
...
for fdout=1:c_out.NumTestSets
    ...
    pre(test(c_out,fdout))=prob_estimates;
    tl(test(c_out,fdout))=Test_lab;
    %fprintf('current fold=%d.\n',fdout);
end
mean(Test_res)

'brainNetSet_SRTL_UNTRC'
save('result_SRTL_UNTIC_001.mat','Test_res','pre','tl','Vali_res')